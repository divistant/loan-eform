...
type SimulatorFieldPurposeProps = {
  product: Product;
  value?: string;
...
export function SimulatorFieldPurpose({
  product,
  value,
  onChange,
  validationMessage,
}: SimulatorFieldPurposeProps) {
  const options = product.simulatorConfig?.options?.purpose || [];
  const hasError = validationMessage && validationMessage.includes("purpose");
  const isValid = value && value.length > 0 && !hasError;

  return (
    <div className="space-y-2">
      <Label htmlFor="purpose" className="text-sm font-semibold text-gray-900 mb-2 block">
      ...
      <div className="relative">
        <Select value={value || ""} onValueChange={onChange}>
          <SelectTrigger
            ...
            aria-describedby={hasError ? "purpose-error" : undefined}
          >
      ...
      {hasError && (
        <p id="purpose-error" className="text-xs text-red-600 flex items-center gap-1.5" role="alert">
